...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ];then
    ...
    in_xlsx=data/2024-7-22_exp/text_rawdata/words.xlsx
    out_txt=data/2024-7-22_exp/text_rawdata/words.txt
    python3 tools/read_xlsx.py $in_xlsx $out_txt
fi
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ];then
   echo "generate input format data of tts server "
   for name in words utts prompt;do
    in_txt=data/2024-7-22_exp/text_rawdata/$name.txt
    out_xml=data/2024-7-22_exp/xml/$name
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ];then
   ...
   for name in words utts prompt;do
       in_xml_dir=data/2024-7-22_exp/xml/$name
       out_audio_dir=data/2024-7-22_exp/audio/$name
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ];then
   ...
   for name in words utts prompt;do
    in_audio_dir=data/2024-7-22_exp/audio/$name
    out_audio_dir=exp/2024-7-22_exp/audio/$name
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ];then
   ...
   for name in words ;do
      in_audio_dir=exp/2024-7-22_exp/audio/$name
      out_audio_dir=exp/2024-7-22_exp/audio/${name}_fix
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ];then
  for name in words ;do
      content_audio_dir=exp/2024-7-22_exp/audio/${name}_fix
      prompt_dir=exp/2024-7-22_exp/audio/prompt_fix
      required_silence_audio_len=2
      ding_path=data/2024-7-22_exp/ding_fix/ding_online_fix.wav
      order_random=False
      concatente_audio_name="1"
      output_dir=exp/2024-7-22_exp/audio/${name}_final
      sample_num=1
